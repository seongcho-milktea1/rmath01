...
```{r coordinates-1axis, echo = FALSE, out.width = '90%', fig.cap = '1차원 직선 위에 표현한 다섯 점의 좌표'}
knitr::include_graphics("images/0401_coordinates-1axis.png")
```
...
```{r leaflet-jeju, echo = FALSE, out.width = '60%', fig.cap = '한라산 좌표와 수채화풍 배경지도를 이용한 제주도 지역의 시각화'}
knitr::include_graphics("images/0402_leaflet-jeju.png")
```
...
```{r coordinates-plane, echo = FALSE, out.width = '70%', fig.cap = '피보나치 사각형과 좌표평면의 사분면'}
knitr::include_graphics("images/0403_coordinates-plane.png")
```
...
```{r milktea3-height-table, echo = TRUE}
library(dplyr)
library(kableExtra)
knitr::kable(
  h_df,
...
```{r milktea3-height-graph, echo = FALSE, out.width = '70%', fig.cap = '학년 별 키의 변화 그래프'}
knitr::include_graphics("images/0404_milktea3-height-graph.png")
```
...
```{r milktea3-height-graph2, echo = FALSE, out.width = '70%', fig.cap = '연도 별 전년도와의 키 차이 (변화량과 변화율)'}
knitr::include_graphics("images/0405_milktea3-height-graph2.png")
```
...
```{r milktea1-walking-table, echo = TRUE}
library(dplyr)
library(kableExtra)
x <- 1:5                              #시간 벡터
...
```{r milktea1-speed-mode-table, echo = TRUE}
library(dplyr)
library(kableExtra)
day <- c(1:3)                               #일수 벡터
...
```{r milktea1-graph2, echo = FALSE, out.width = '120%', fig.cap = '시간과 거리의 정비례 그래프와 속력과 시간의 반비례 그래프'}
knitr::include_graphics("images/0406_milktea1-graph1and2.png")
```
...
```{r proportion-a-graph, echo = FALSE, out.width = '120%', fig.cap = 'a값에 따른 정비례 그래프와 반비례 그래프의 변화'}
knitr::include_graphics("images/0407_proportion-a-graph1and2.png")
```
