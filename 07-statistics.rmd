...
```{r pm10-sido-2019-03, echo = FALSE, out.width = '100%', fig.cap = '$18$개 시도 별 미세먼지 농도 (2019년 3월) 데이터의 박스 플롯'}
knitr::include_graphics("images/0701_pm10-sido-2019-03.png")
```
...
```{r pm10-gyeonggi-2019-03, echo = FALSE, out.width = '25%', fig.cap = '경기도 미세먼지 농도 (2019년 3월) 박스 플롯과 $31$개의 변량'}
knitr::include_graphics("images/0702_pm10-gyeonggi-2019-03.png")
```
...
```{r pm10-stem-leaf-table, echo=FALSE, eval=TRUE, results="hold"}
library(dplyr)
library(kableExtra)
options(knitr.kable.NA = '')
pm10_df3$pm10
knitr::kable(
  data.frame(stem_leaf_df2[1:9,],stem_leaf_df2[10:18,]),
  col.names = rep(c('줄기','잎'),2),	 #표 컬럼 이름 설정
  booktabs = TRUE, align = "c",      #표 위치 설정
  caption = '줄기 (십의 자리)와 잎 (일의 자리) 그림') %>%  #표 제목 설정
...
```{r iris-3species, echo = FALSE, out.width = '70%', fig.cap = '아이리스 3종 <font size="2">(출처 <https://thegoodpython.com/iris-dataset>)</font>'}
knitr::include_graphics("images/0703_iris-3species.png")
```

```{r iris-sepal-petal, echo = FALSE, out.width = '35%', fig.cap = '아이리스 꽃받침과 꽃잎의 길이와 너비 <font size="2">(출처 <https://rebrand.ly/integratedots-iris>)</font>'}
knitr::include_graphics("images/0704_iris-sepal-petal.png")
```

아이리스 데이터는 다섯 개의 컬럼을 가지고 있고 $150$행으로 구성되어있기 때문에 이해하기가 쉽고 크기도 작아서 통계의 기본 개념을 설명할 때 널리 사용됩니다 (표 \@ref(tab:iris-cols-table)).

```{r iris-cols-table, echo=FALSE, eval=TRUE}
library(dplyr)
library(kableExtra)
#컬럼명, 의미, 데이터 타입
r1 <- c("Species", "붓꽃의 종", "Factor")
r2 <- c("Sepal.Width", "꽃받침의 너비", "Number")
r3 <- c("Sepal.Length", "꽃받침의 길이", "Number")
r4 <- c("Petal.Width", "꽃잎의 너비", "Number")
r5 <- c("Petal.Length", "꽃잎의 길이", "Number")
col_df <- rbind(r1,r2,r3,r4,r5)
colnames(col_df) <- c('column_name','meaning','data_type')
rownames(col_df) <- c()
knitr::kable(
  col_df,
  col.names = c('컬럼명','의미','데이터 타입'),  #표 컬럼 이름 설정
  booktabs = TRUE, align = "c",            #표 위치 설정
  caption = 'iris 데이터의 컬럼 정보') %>%  #표 제목 설정
kable_styling(full_width=FALSE,font_size=15)       #표 컬럼 간격 설정
```
...
```{r iris-3species-boxplot, echo = FALSE, out.width = '45%', fig.cap = '아이리스 3종의 꽃잎 길이 박스 플롯'}
knitr::include_graphics("images/0705_iris-3species-boxplot.png")
```
...
```{r iris-frequency-distribution-table, echo=FALSE, eval=TRUE}
library(dplyr)
library(kableExtra)
iris_frequency_distribution_df <- iris %>% 
	mutate(class = cut(Sepal.Width,breaks=12,,right=FALSE)) %>%
	mutate(classname = gsub("\\[|\\)", "", class)) %>%
	separate(classname, into=c("from", "to"),sep =',') %>%
	mutate(from = format(as.numeric(from), nsmall = 1)) %>%
	mutate(to = format(as.numeric(to), nsmall = 1)) %>%
	mutate(classname = paste0(from,'이상 ~ ',to,'미만')) %>%
	group_by(classname) %>%
	count() 
knitr::kable(
  iris_frequency_distribution_df,
  col.names = c('계급','도수'),                     #표 컬럼 이름 설정
  booktabs = TRUE, align = "c",                   #표 위치 설정
  caption = '아이리스 꽃받침 너비 변량의 도수분포표') %>%   #표 제목 설정
kable_styling(full_width=FALSE,font_size=15) %>%  #표 컬럼 간격 설정
...
```{r sepal-width-frequency, echo = FALSE, out.width = '70%', fig.cap = '아이리스 꽃받침 너비 히스토그램과 도수분포다각형'}
knitr::include_graphics("images/0706_sepal-width-frequency.png")
```
...
```{r iris-relative-frequency-table, echo=TRUE, eval=TRUE}
library(dplyr)
library(kableExtra)
knitr::kable(
...
```{r sepal-width-relative-frequency, echo = FALSE, out.width = '70%', fig.cap = '아이리스 꽃받침 너비 상대도수'}
knitr::include_graphics("images/0707_sepal-width-relative-frequency.png")
```
...
```{r sepal-width-2species-relative-frequency-table, echo=TRUE, eval=TRUE}
library(dplyr)
library(tidyr)
library(kableExtra)
...
```{r sepal-width-2species-relative-frequency, echo = FALSE, out.width = '70%', fig.cap = '아이리스 versicolor와 virginica의 꽃받침 너비 상대도수 도수분포다각형'}
knitr::include_graphics("images/0708_sepal-width-2species-relative-frequency.png")
```
