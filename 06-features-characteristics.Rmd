...
```{r polygon-diagonal, echo = FALSE, out.width = '80%', fig.cap = '다각형의 대각선'}
knitr::include_graphics("images/0601_polygon-diagonal.png")
```

대각선의 개수를 구하는 공식 $\frac{n(n-3)}{2}$을 이용하면 표 \@ref(tab:sides-diagonals-table)과 같은 정보를 얻을 수 있습니다. 표를 생성하는 데 사용된 소스코드는 아래와 같습니다. 

```{r sides-diagonals-table, echo = TRUE}
library(dplyr)
library(kableExtra)
sides <- 3:20                            
...
```{r polygon-internal-angle, echo = FALSE, out.width = '80%', fig.cap = '다각형의 내각의 크기의 합'}
knitr::include_graphics("images/0602_polygon-internal-angle.png")
```
...
```{r polygon-exterior-angle, echo = FALSE, out.width = '40%', fig.cap = '삼각형의 내각과 외각'}
knitr::include_graphics("images/0603_polygon-exterior-angle.png")
```
...
```{r polygon-exterior-angle-sum, echo = FALSE, out.width = '40%', fig.cap = '다각형의 외각의 크기의 합'}
knitr::include_graphics("images/0604_polygon-exterior-angle-sum.png")
```
...
```{r triangle-internal-exterior, echo = FALSE, out.width = '40%', fig.cap = '삼각형의 한 외각의 크기와 이웃하는 않는 두 내각의 크기의 합'}
knitr::include_graphics("images/0605_triangle-internal-exterior.png")
```
...
```{r circle-arc-chord, echo = FALSE, out.width = '35%', fig.cap = '원의 호와 현'}
knitr::include_graphics("images/0606_circle-arc-chord.png")
```
...
```{r circle-sector-segment, echo = FALSE, out.width = '35%', fig.cap = '부채꼴과 활꼴'}
knitr::include_graphics("images/0607_circle-sector-segment.png")
```
...
```{r sector-same-angle, echo = FALSE, out.width = '100%', fig.cap = '중심각이 같은 두 부채꼴의 호의 길이과 넓이'}
knitr::include_graphics("images/0608_sector-same-angle.png")
```
...
```{r sector-direct-proportion, echo = FALSE, out.width = '30%', fig.cap = '한 원에서 부채꼴의 중심각과 호의 길이, 중심각과 넓이와의 정비례 관계'}
knitr::include_graphics("images/0609_sector-direct-proportion.png")
```
...
```{r cube, echo = FALSE, out.width = '120%', fig.cap = '여섯 개의 정사각형을 면으로 가지는 정육면체'}
knitr::include_graphics("images/0610_cube1234.png")
```
...
```{r hexahedron, echo = FALSE, out.width = '120%', fig.cap = '여섯 개의 삼각형을 면으로 가지는 육면체'}
knitr::include_graphics("images/0611_hexahedron1234.png")
```
...
```{r triangular-prism, echo = FALSE, out.width = '120%', fig.cap = '삼각형 밑면 (아랫면과 윗면)을 가지는 오면체 삼각기둥'}
knitr::include_graphics("images/0612_triangular-prism1234.png")
```
...
```{r triangular-pyramid, echo = FALSE, out.width = '120%', fig.cap = '네 개의 정삼각형을 면으로 가지는 정사면체'}
knitr::include_graphics("images/0613_triangular-pyramid1234.png")
```
...
```{r truncated-quadrangular-pyramid, echo = FALSE, out.width = '120%', fig.cap = '육면체인 사각뿔대'}
knitr::include_graphics("images/0614_truncated-quadrangular-pyramid1234.png")
```
...
```{r regular-polyhedron-table, echo = FALSE}
#knitr::opts_chunk$set(echo = TRUE, warning=FALSE, message=FALSE)
library(dplyr)
library(kableExtra)
polyhedron <- c('정사면체','정팔면체','정이십면체','정육면체','정십이면체')  #정다면체
shape_face <- c('정삼각형','정삼각형','정삼각형','정사각형','정오각형')  #면의 모양
n_face <- c(4,8,20,6,12)  #면의 개수
n_face_for_each_vertex <- c(3,4,5,3,3)  #각 꼭짓점에 모인 면의 개수
n_vertex <- c('4','6<br>(2 x 3)','12<br>(4 x 3)','8','20<br>(8 + 4 x 3)')  #꼭짓점의 개수
coor_vertex <- c('(1,1,1) <br> (1,−1,−1)<br>(−1,1,−1)<br>(−1,−1,1)',  #꼭지점 좌표
                 '(±1,0,0)<br>(0,±1,0)<br>(0,0,±1)',
                 '(0,±1,±𝜙)<br>(±1,±𝜙,0)<br>(±𝜙,0,±1)','(±1,±1,±1)',
                 '(±1,±1,±1)<br>(0,±1/𝜙,±𝜙)<br>(±1/𝜙,±𝜙,0)<br>(±𝜙,0,±1/𝜙)')
images <- c('Tetrahedron','Octahedron','Icosahedron','Hexahedron','Dodecahedron')
png_images <- sprintf("![](%s){width=100px}", paste0('./images/0602t_',images,'.png'))
gif_images <- sprintf("![](%s){width=150px}", paste0('./images/0602t_',images,'.gif'))
polyhedron_df <- data.frame(polyhedron,shape_face,n_face,
                            n_face_for_each_vertex,n_vertex,coor_vertex,png_images,gif_images)
#좀 더 읽기 쉬운 표를 만들기 위해 행과 열을 바꿈
polyhedron_df2 <- as.data.frame(t(polyhedron_df))
#첫 행을 컬럼 이름으로 사용 
colnames(polyhedron_df2) <- as.character(unlist(polyhedron_df2[1,]))
polyhedron_df2 <- polyhedron_df2[-1,]
rownames(polyhedron_df2) <- c('면의 모양','면의 개수','한 꼭짓점에 모인<br>면의 개수','꼭짓점<br>개수','꼭짓점<br>좌표','그림','움직이는<br>그림')
...
knitr::kable(
  ...
  escape=FALSE,
  booktabs = TRUE, align = "c",      #표 위치 설정
  caption = '정다면체의 성질') %>%       #표 제목 설정
kable_styling(full_width=FALSE,font_size=15) %>%     #표 컬럼 간격 설정
column_spec(1, border_right=TRUE, include_thead=FALSE, 
  extra_css="border-right:double lightgrey;") %>% 
add_footnote('표에 포함된 그림의 출처 https://en.wikipedia.org/wiki/Platonic_solid',
  notation="symbol")
```
...
```{r cylinder-cone-sphere, echo = FALSE, out.width = '60%', fig.cap = '원기둥, 원뿔, 그리고 구'}
knitr::include_graphics("images/0615_cylinder-cone-sphere.png")
```
...
```{r truncated-cone, echo = FALSE, out.width = '30%', fig.cap = '원뿔대'}
knitr::include_graphics("images/0616_truncated-cone.png")
```
...
```{r triangular-pris-development-figure, echo = FALSE, out.width = '40%', fig.cap = '삼각기둥의 전개도'}
knitr::include_graphics("images/0617_triangular-pris-development-figure.png")
```
...
```{r cylinder-development-figure, echo = FALSE, out.width = '30%', fig.cap = '원기둥의 전개도'}
knitr::include_graphics("images/0618_cylinder-development-figure.png")
```
...
```{r quadrangular-pyramid-development-figure, echo = FALSE,out.width = '35%', fig.cap = '사각뿔과 사각뿔의 전개도'}
knitr::include_graphics(c("images/0619_a_quadrangular-pyramid.png", "images/0619_b_quadrangular-pyramid-development-figure.png"))
```
...
```{r sphere-mesh, echo = FALSE, out.width = '35%', fig.cap = '구를 한없이 많은 각뿔 모양으로 분해'}
knitr::include_graphics(c("images/0620_a_sphere-mesh.png","images/0620_b_sphere-mesh-cell.png"))
```
...
```{r plane-to-3d2, echo = FALSE, out.width = '50%', fig.cap = '한반도 주변의 높이 (고도) 정보를 이용한 삼차원 시각화'}
knitr::include_graphics("images/0621_plane-to-3d2.png")
```
...
```{r globe1, echo = FALSE, out.width = '50%', fig.cap = '지구 삼차원 시각화'}
knitr::include_graphics("images/0622_globe1.png")
```
